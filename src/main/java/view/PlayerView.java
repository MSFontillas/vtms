...
public class PlayerView {
    ...
    public Player getUpdatePlayerInput(Player existingPlayer) {
        Player player = new Player();

        System.out.println("Enter player name (press Enter to keep current: " + existingPlayer.getPlayerName() + "): ");
        String playerName = scanner.nextLine();
        player.setPlayerName(playerName.isEmpty() ? existingPlayer.getPlayerName() : playerName);

        System.out.println("Enter IGN (press Enter to keep current: " + existingPlayer.getIgn() + "): ");
        String ign = scanner.nextLine();
        player.setIgn(ign.isEmpty() ? existingPlayer.getIgn() : ign);

        System.out.println("Enter role (press Enter to keep current: " + existingPlayer.getRole() + "): ");
        String role = scanner.nextLine();
        player.setRole(role.isEmpty() ? existingPlayer.getRole() : role);

        System.out.println("Enter teamID (press Enter to keep current: " + existingPlayer.getTeamID() + "): ");
        String teamIdInput = scanner.nextLine();
        player.setTeamID(teamIdInput.isEmpty() ? existingPlayer.getTeamID() : Integer.parseInt(teamIdInput));
    ...
    public void displayPlayers(List<Player> players) {
        System.out.println("\n--- Players List ---");
        System.out.printf("| %-5s | %-20s | %-15s | %-10s | %-30s |\n",
                "ID", "Player Name", "IGN", "Role", "Team");
        System.out.println("-".repeat(90));

        for (Player player : players) {
            String teamInfo = player.getTeamID() + " - " + 
                         (player.getTeamName() != null ? player.getTeamName() : "N/A");
        System.out.printf("| %-5d | %-20s | %-15s | %-10s | %-30s |\n",
                player.getPlayerID(),
